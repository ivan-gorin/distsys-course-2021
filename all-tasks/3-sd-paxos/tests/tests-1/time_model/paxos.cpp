...
class PaxosServerTimeModel : public IServerTimeModel {
 public:
  // Clocks
...
class PaxosTimeModel : public ITimeModel {
 public:
  void Initialize() override {
    slow_message_freq_ = GlobalRandomNumber(1, 7);
    slow_message_delay_ = GlobalRandomNumber(200, 800);
  }
  ...
  IServerTimeModelPtr MakeServerModel(const std::string& /*host*/) override {
    return std::make_unique<PaxosServerTimeModel>();
  }
...
ITimeModelPtr MakeTimeModel() {
  return std::make_unique<PaxosTimeModel>();
}
