...
#include <tests/time_models/async_2.hpp>

#include "../common/leader_tracker.hpp"
...
void NetAdversary() {
  ...
  while (matrix::GlobalNow() < kNoMoreFaults) {
    ...
    matrix::fault::RandomPause(500_jfs, 3000_jfs);
...
void LeaderAdversary() {
  ...
  LeaderTracker leader_tracker(rpc_port);

  while (matrix::GlobalNow() < kNoMoreFaults) {
    node::rt::SleepFor(10_jfs);
    auto leader = leader_tracker.Track();

    if (leader.has_value()) {
      // Reboot some follower
      while (node::rt::RandomNumber(2) == 0) {
        matrix::fault::RandomServer(pool).FastReboot();
      }

      auto& leader_server = matrix::fault::Server(*leader);

      if (!leader_server.IsAlive()) {
        continue;
      }

      if (node::rt::RandomNumber(2) == 0) {
        await::fibers::Go([]() {
          MakePartition();
        });
      }

      matrix::fault::RandomPause(1_jfs, 100_jfs);

      int choice = node::rt::RandomNumber(1, 5);

      if (choice <= 3) {
        LOG_INFO("Reboot leader {}", *leader);
        leader_server.FastReboot();
        leader_tracker.Reset();
      } else {
        LOG_INFO("Pause leader {}", *leader);
        leader_server.Pause();
        matrix::fault::RandomPause(500_jfs, 2000_jfs);
...
size_t RunSimulation(size_t seed) {
  ...
  world.SetTimeModel(tests::MakeAsyncTimeModel2());
