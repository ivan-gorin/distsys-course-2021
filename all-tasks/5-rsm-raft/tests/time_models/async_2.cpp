 ...
 public:
  ...
  int InitClockDrift() override {
    if (GlobalRandomNumber() % 2 == 0) {
      // Relatively fast
      return 10 + GlobalRandomNumber(25);
    } else {
      // Relatively slow
      return -10 - (int)GlobalRandomNumber(10 + 1);
    }
  }
...
class TimeModel : public ITimeModel {
 public:
  void Initialize() override {
    ...
    slow_message_delay_ = GlobalRandomNumber(100, 200);
  }
  ...
  virtual Jiffies EstimateRtt() const override {
    return 250;  // Backward compatibility
  }
...
ITimeModelPtr MakeAsyncTimeModel2() {
